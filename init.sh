...
(cd pre-processing/pre-processing-code && rm -r ./pre-processing-code.zip || true; zip -r pre-processing-code.zip .)
...
then
  ...
  echo "Please create the ADX product manually and update the pre-processing cloudformation stack with the right product id"
else
